...
import { Box, Grid, TextField, InputAdornment, IconButton, Typography, Tabs, Tab, Backdrop, CircularProgress, Snackbar } from '@mui/material'
    ...
    <Layout history={props.history} page="Productos" user={props.user?.uid}>
      <Box sx={{ maxWidth: 1200, mx: 'auto', p: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
          <TextField
            fullWidth size="small" placeholder="Buscar producto..."
          ...
          <Grid container spacing={2}>
            {filtered.map(([name, p]) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={name}>
                <CardProducto
                  name={name} precio={p.precio} cantidad={p.cantidad}
                  isSubproducto={p.isSubproducto} onDelete={eliminar}
                />
              </Grid>
            ))}
          </Grid>
