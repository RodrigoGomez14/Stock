  ...
  const RecibirEntrega=(props)=>{
    ...
    const agregarPagoAlHistorial = pago =>{
        ...
        if((efectivo?parseFloat(efectivo):0)+total){
            let newPagos = props.proveedores[props.entregas[id].proveedor].pagos?props.proveedores[props.entregas[id].proveedor].pagos:[]
            newPagos.push(pago)
            database().ref().child(props.user.uid).child('proveedores').child(props.entregas[id].proveedor).child('pagos').update(newPagos)
        }
    }
    return(
