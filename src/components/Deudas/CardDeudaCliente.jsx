...
const CardDeudaCliente = ({ nombre, deuda, search }) => {
  ...
  return (
    <Card sx={{ borderRadius: 2, display: 'flex', flexDirection: 'column', height: '100%', transition: '0.2s', '&:hover': { transform: 'translateY(-2px)', boxShadow: 4 } }}>
      <CardContent sx={{ flex: 1, pb: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
          <Typography variant="body1" fontWeight={700}
            component={Link} to={`/Cliente?${encodeURIComponent(nombre)}`}
            sx={{ textDecoration: 'none', color: 'inherit', '&:hover': { color: 'primary.light' }, flex: 1 }}>
            {nombre}
          </Typography>
          <IconButton size="small" component={Link} to={`/Editar-Cliente?${encodeURIComponent(nombre)}`}
      ...
      <Box sx={{ display: 'flex', borderTop: '1px solid', borderColor: 'divider' }}>
        <Button component={Link} to={`/Historial-Cliente?${encodeURIComponent(nombre)}`}
          startIcon={<History />} fullWidth size="small"
          sx={{ borderRadius: 0, py: 1.2, fontWeight: 500, fontSize: 11 }}>
          Historial
        </Button>
        <Button component={Link} to={`/Nuevo-Pago-Cliente?${encodeURIComponent(nombre)}`}
